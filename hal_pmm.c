...
unsigned int SetVCore(unsigned char level) {
  ...
  while (((level != actlevel) && (status == 0)) ||
         (level < actlevel)) // step by step increase or decrease
  {
    if (level > actlevel)
...
unsigned int SetVCoreUp(unsigned char level) {
  ...
  // Set also SVS highside to new level		       //-> Vcc is high enough
  // for a Vcore increase
  SVSMHCTL |= SVSHE | (SVSHRVL0 * level);
  // Set SVM low side to new level
  SVSMLCTL = SVMLE | SVMLFP | (SVSMLRRL0 * level);
  SVSMLCTL |=
      SVSLMD; // also set SVSLMD bit when Full Performance mode is enabled
...
unsigned int SetVCoreDown(unsigned char level) {
  ...
  if ((PMMIFG & SVMHIFG) == SVMHIFG)
    return PMM_STATUS_ERROR; // Highside is still to low for the adjusted VCore
                             // Level
  else
    return PMM_STATUS_OK; // Return: OK
}
