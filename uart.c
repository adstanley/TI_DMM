...
#include "uart.h"
#include "defines.h" // for mode defines
#include "main.h"    // for measurement_mode, displayed_result_fp, etc.
...
void uart_print_measurement(void) {
  const char *mode_str[] = {
      [DC_VOLTAGE] = "DC V", [AC_VOLTAGE] = "AC V", [DC_CURRENT] = "DC I",
      [AC_CURRENT] = "AC I", [POWER] = "POWR",      [OFF] = "OFF "};
  ...
  printf("%s  %8.4f %s%s\r\n", mode_str[measurement_mode], displayed_result_fp,
         unit, (range_overflow > 0) ? "  OL" : "");
}